...
import { getProducts } from "app/store/homeSlice";
...
function FuseSearch(props) {
  ...
  function renderSuggestion(suggestion, { query, isHighlighted }) {
    ...
    return (
      <MenuItem
        ...
        component="div"
        onClick={() => {
          History.push(`/shop/product/${suggestion.id}`);

          dispatch({
            type: "setSearchText",
            value: suggestion.title,
          });
          state.opened && hideSearch();
        }}
      >
  ...
  function handleChange(event) {
    reduxDispatch(getProducts(`limit=20&title=${event.target.value}&minPrice=100`));
    dispatch({
      type: "setSearchText",
